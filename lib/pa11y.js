...
async function runPa11yTest(url, options, state) {
	...
	// change between runs
	if (!runnerJavaScript.pa11y) {
		runnerJavaScript.pa11y = await fs.readFile(`${__dirname}/runner.js`, 'utf-8');
	}
	for (const runner of options.runners) {
		if (!runnerJavaScript[runner]) {
	...
	await page.evaluate(runnerJavaScript.pa11y);
	for (const runner of options.runners) {
		options.log.debug(`Injecting runner: ${runner}`);
		await page.evaluate(runnerJavaScript[runner]);
	}
